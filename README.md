...
2. В **SQL Editor** выполнить по порядку файлы из `supabase/migrations/`:
   ...
   - `20260721130000_add_auth_and_crm.sql` — аутентификация, роли, личный кабинет клиента, CRM админа (см. `docs/BACKEND.md`).
...
После того как нужный человек зарегистрируется через сайт (`/register`), назначьте ему
роль админа в **SQL Editor**:

```sql
update public.profiles set role = 'admin' where email = 'you@example.com';
```

После этого в шапке сайта у него появится пункт «CRM» (`/admin`). Подробности — `docs/BACKEND.md`.
...
- [`docs/BACKEND.md`](docs/BACKEND.md) — схема БД, роли, RLS, как назначить админа.
- [`docs/FRONTEND.md`](docs/FRONTEND.md) — роуты, auth-стейт, структура фич.
...
- Кнопки WhatsApp/Instagram, плавающая кнопка WhatsApp, баннер-приглашение в WhatsApp-группу под хедером.
- 7 туров-заглушек (Турция, ОАЭ, Египет, Вьетнам, Сингапур, Сейшелы, Европа) — фото со стоков Unsplash.
- **Регистрация и вход** (email + пароль) — заявку по-прежнему можно оставить и без аккаунта.
- **Личный кабинет клиента** (`/account`) — профиль, история своих заявок, избранные туры
  (сердечко на карточке тура).
- **CRM для администратора** (`/admin`, доступ только по роли) — заявки (смена статуса), туры
  (создание/редактирование/удаление), модерация отзывов, FAQ и блог. Подробности — `docs/BACKEND.md`, `docs/FRONTEND.md`.
...
- **Восстановление пароля и смена email** — не реализованы в этой итерации.
- **Загрузка фото файлом в CRM** — сейчас поле URL (тот же паттерн, что и раньше), Storage +
  аплоад с компрессией — логичный следующий шаг.
